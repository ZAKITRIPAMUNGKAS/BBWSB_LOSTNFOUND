...
function parsingdata($data, $jumlah)
{
	...
	// print_r(count($merge_parameter));
	// print_r(count($jumlah));
	// print_r(array_unique($merge_parameter));
	// exit;
	$b = [];
	for ($i = 0; $i < count(array_unique($merge_parameter)); $i++) {
		foreach ($a as $key => $value) {
			if ((int)count($data) !== (int)count($jumlah)) {
				for ($x = 1; $x < count($jumlah); $x++) {
